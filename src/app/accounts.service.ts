...
export class AccountsService {
  ...
  registerSymbol(symbol, contract, amount, type) {
    ...
    if (idx === -1) {
      this.tokens.push({
        name: symbol,
        contract: contract,
        //balance: type == 'sent' ? 0 - amount : amount,
        balance: 'Loading...',
        price: null,
        loading: true
      });
  ...
  getTokenBalances() {
    this.tokens.forEach((tk, index) => {
      try {
        this.eos.eos['getCurrencyBalance'](tk.contract, this.selected.getValue().name, tk.name).then((tokendata) => {
          ...
          if (this.tokens[index]) {
            ...
            this.tokens[index].loading = false;
          }
        });
      } catch (error) {
        console.log('Error in method getTokenBalances');
        console.log(error);
      }
    });
  }

  async reloadActions(account) {
    try {
      if (!account || !this.eos.eos) {
        console.log('Error in method reloadActions in account service');
        console.log('Account is undefinded');
        return false;
      }
      this.actions = [];
      this.tokens = [];
      this.loading = true;
      var data = await this.eos.getActions({
      ...
      data.actions.forEach((item) => {
        ...
        if (action_name === 'transfer') {
          if (!act['data']['quantity']) {
            act['data'] = this.eos.decode('transfer', act['data']);
          }
          ...
          if (contract === 'eosio.token') {
            ...
            this.registerSymbol(symbol, contract, amount, type);
          }
        }
      ...
      this.actions = this.actions.sort((a, b) => { return Date.parse(b.date) - Date.parse(a.date); });
      this.accounts[this.selectedIdx]['tokens'] = this.tokens;
      this.accounts[this.selectedIdx]['actions'] = this.actions;
      this.getTokenBalances();
      this.loading = false;
    } catch (error) {
      console.log('Error in method reloadActions with params ' + account);
      console.log(error);
    }
  }
  ...
  select(index) {
    ...
    if (sel['actions']) {
      if (sel.actions.length > 0) {
        this.actions = sel.actions.sort((a, b) => { return Date.parse(b.date) - Date.parse(a.date); });
      }
    } else {
      this.actions = [];
    }
  ...
  initFirst(index = 0) {
    this.selectedIdx = index;
    this.selected.next(this.accounts[index]);
  }
  ...
  importAccounts(accounts) {
    ...
    localStorage.setItem('hila.accounts.' + chain_id, JSON.stringify(payload));
    this.loadLocalAccounts(accounts);
  }

  appendNewAccount(account) {
    const chain_id = this.eos.chainID;
    const payload = JSON.parse(localStorage.getItem('hila.accounts.' + chain_id));
