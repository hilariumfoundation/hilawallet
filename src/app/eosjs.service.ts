...
export class EOSJSService {
  ...
  init(url, chain) {
    ...
    if (localStorage.length == 0 && window.location.href.toString().search('landing') == -1) {
      const linkold = window.location.href.split('#')[0].toString();
      const linknew = linkold + '#/landing';
      ...
      return new Promise((resolve, reject) => {
        ...
        this.eos['getInfo']({}).then(result => {
          ...
          let savedAcc = [];
          const savedpayload = localStorage.getItem('hila.accounts.' + this.chainID);
          if (savedpayload) {
  ...
  decode(method, data) {
    return this.eos.fc.fromBuffer(method, data);
  }
  ...
  storeAccountData(accounts) {
    if (accounts) {
      if (accounts.length > 0) {
        ...
        const payload = JSON.parse(localStorage.getItem('hila.accounts.' + this.chainID));
        payload.updatedOn = new Date();
        payload.accounts = accounts;
        localStorage.setItem('hila.accounts.' + this.chainID, JSON.stringify(payload));
      }
    }
  }
  ...
  saveHistory() {
    ...
    localStorage.setItem('hila.txhistory.' + this.chainID, payload);
  }
  ...
  getActions(data){
    return this.eos['getActions'](data);
  }
