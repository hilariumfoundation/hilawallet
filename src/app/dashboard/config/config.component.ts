...
export class ConfigComponent implements OnInit {
  ...
  logout() {
    ...
    } else 
    {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        if (localStorage.key(i) !== 'hila.contacts') {
  ...
  async changePass() {
    ...
    if (this.passmatch) {
      ...
      let authenticate = await this.crypto.authenticate(this.passForm.value.oldpass, publicKey);
  ...
  setPIN() {
    if (this.pinForm.value.pin !== '') {
      if (localStorage.getItem('hila-hash')) {
