...
export class VotingService {
  ...
  getDataListProducers(producers, global) {
    this.totalActivatedStake = parseInt(global.rows[0]['total_activated_stake'], 10) / 10000;
  ...
  async listProducers() {
    if (this.initList === false) {
      ...
      var producers = null;
      var global = null;
      while (producers == null || global == null) {
        try {
          producers = await this.eos.listProducers();
          global = await this.eos.getChainInfo();
          this.getDataListProducers(producers, global);
        } catch (error) {
          console.log('It is loading for list producers ...');
          await UtilityService.sleep(1000);
        }
      }
    }
  };
