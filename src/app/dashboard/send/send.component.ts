...
export class SendComponent implements OnInit {
  ...
  filter(val: string, indexed): string[] {
    let data = this.contacts.filter(contact => {
      if (contact.type === 'contact') {
        let data = contact.name.toLowerCase().includes(val.toLowerCase()) || contact.account.toLowerCase().includes(val.toLowerCase());
        return data;
      } else {
        if (indexed) {
    ...
    if (data.find(x => x.account !== undefined) === undefined) {
    ...
    this.aService.selected.asObservable().subscribe((sel: EOSAccount) => {
      if (sel) {
        ...
        try {
          ...
          for (let item in sel.tokens) {
            if (sel.tokens[item]["balance"].split(' ')[1] == this.sendForm.value.token) {
              this.token_balance = sel.tokens[item]["balance"].split(' ')[0];
            }

            // if (typeof this.aService.tokens[0]["balance"] === 'undefined') {
            //   this.token_balance = 0;
            // }
            // else {
            //   this.token_balance = this.aService.tokens[0]["balance"].split(' ')[0];
          }
        }
        catch (ex) {
    ...
    this.sendForm.get('token').valueChanges.subscribe((symbol) => {
      ...
      if (symbol !== 'EOS') {
        try {
          this.token_balance = 0;
          this.aService.selected.asObservable().subscribe((sel: EOSAccount) => {
            const tk_idx = sel.tokens.findIndex((val) => {
              return val.name === symbol;
            });
            if (tk_idx > -1) {
              this.selectedToken = sel.tokens[tk_idx];
              try {
  ...
  setMax() {
    this.sendForm.patchValue({
      amount: Math.floor((this.sendForm.get('token').value === 'EOS' ? this.unstaked : this.token_balance) * 10000) / 10000
    });
  }
  ...
  storeContacts() {
    localStorage.setItem('hila.contacts', JSON.stringify(this.contacts));
  }

  loadContacts() {
    const contacts = localStorage.getItem('hila.contacts');
    if (contacts) {
      ...
      this.crypto.authenticate(this.confirmForm.get('pass').value, publicKey).then((res) => {
        if (res === true) {
          ...
          this.eos.transfer(contract, from, to, parseFloat(amount).toFixed(4) + ' ' + tk_name, memo).then((result) => {
            ...
            //reset money
            try {
              if (this.sendForm.value.token == 'EOS') {
                this.unstaked = this.unstaked - amount;
              }
              else {
                this.token_balance = this.token_balance - amount;
              }
            }
            catch (ex) {
