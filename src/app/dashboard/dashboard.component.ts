...
export class DashboardComponent implements OnInit {
    ...
    //NEW ACCOUNT
    newAccOptions = 'thispk';              //Method create new account                
    accountname = '';
    ...
    constructor(
        ...
        this.delegateForm = this.fb.group({
            delegate_amount: [0.2, [Validators.required, Validators.min(0.2)]],
        ...
        this.crypto.authenticate(this.submitTXForm.get('pass').value, publicKey).then((data) => {
            if (data === true) {
                this.eos.createAccount(
                    ...
                    delegate_transfer, gift_amount, 'created with hila').then((txdata) => {
                        console.log(txdata);
                        if (this.newAccOptions === 'newpk') {
                            setTimeout(() => {
    ...
    async ngOnInit() {
        // if (this.network.aService.accounts.length == 0) {
        ...
        this.accounts = [];
        console.log('It is waiting connect to network');
        this.ram.reload();
