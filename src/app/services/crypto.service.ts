...
export class CryptoService {
  ...
  createPIN(pin: string) {
    if (pin !== '') {
      ...
      localStorage.setItem('hila-salt', JSON.stringify(salt));
      localStorage.setItem('hila-hash', hash);
    }
    // this.lock();
  }

  unlock(pin: string, target: string[]): boolean {
    const saved_hash = localStorage.getItem('hila-hash');
    const salt = JSON.parse(localStorage.getItem('hila-salt'));
    const hash = CryptoJS.PBKDF2(pin, salt, {keySize: 512 / 32, iterations: 1000}).toString();
    if (hash === saved_hash) {
      this.locked = false;
  ...
  removePIN() {
    localStorage.removeItem('hila-salt');
    localStorage.removeItem('hila-hash');
    this.locked = false;
  }
